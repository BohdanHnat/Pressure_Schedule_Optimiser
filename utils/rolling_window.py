...
Три основних завдання:
  ...
  3. generate_forecast(model_results, window) — 24-годинний прогноз попиту.

ВАЖЛИВО (демо-режим):
  generate_forecast() прогнозує наступні 24 години від КІНЦЯ навчального набору,
  а НЕ від поточної дати.
"""
...
def warm_start_retrain(window: pd.DataFrame | None = None) -> object:
    ...
    Конфігурація SARIMAX (з config.py):
      order          = (1, 0, 1)
...
def generate_forecast(model_results, window: pd.DataFrame | None = None) -> np.ndarray:
    ...
    набору (останньої мітки часу в Sarimax_Historical_Window.csv), а НЕ від
    поточної дати — це усуває часовий розрив між навчальними даними та прогнозом.
